...
router.get("/dashboard", requireAuth, async (req, res) => {
  try {
    ...
    for (const r of allResidents) {
      let deuda = 0;
      for (let m = 0; m < 12; m++) {
        if (r.pagos25[m] === "pendiente") deuda += 350;
      }
      for (let m = 0; m < maxMes2026; m++) {
        if (r.pagos26[m] === "pendiente") deuda += 400;
      }
...
router.get("/morosos", requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      "SELECT * FROM residents ORDER BY calle, mza, lote"
    );
    ...
    console.error("[admin/morosos]", err.message);
...
router.post("/notify-moroso", requireAuth, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      "SELECT * FROM residents WHERE id = $1", [residentId]
    );
    const r = rows[0];
    if (!r) return res.status(404).json({ error: "Residente no encontrado" });
    if (!r.telefono) return res.status(400).json({ error: "Este residente no tiene telefono registrado" });
    ...
    console.error("[admin/notify-moroso]", err.message);
